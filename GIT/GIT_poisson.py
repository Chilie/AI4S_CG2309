...
data = mat73.loadmat(prefix + "Poisson_Triangular_2rd_5000.mat")
...
# path to save model
if cfg.state=='train':
    path = 'training/2GIT/2GIT_' + string
    if not os.path.exists(path):
        os.makedirs(path)
    writer = SummaryWriter(log_dir=path)

    path_model = "./model/2GIT/"
    ...
    if (cfg.path_model):
        ...
        model_state_dict = torch.load('./model/2GIT/2GIT_' + string +  '.model', map_location=device)
        model.load_state_dict(model_state_dict)
    num_epoches = 1
    batch_size = 1
...
# save worse case
    if cfg.state=='eval':
        savemat('predictions/2GIT/2GIT_error_' + string + '.mat',
        ...
        savemat('predictions/2GIT/2GIT_select_' + string + '_id' + str(idx_median) + '.mat',
        ...
        # savemat('predictions/GIT/GIT_da_max_' + string + '_id' + str(idx_max) + '.mat',
        #         {'input': input, 'output': output, 'output_true': output_true})

# save model
if cfg.state=='train':
    torch.save(model.state_dict(), './model/2GIT/2GIT_' + string + '.model')
