...
parser.add_argument('--dim_out', type=int, default=5, help='Number of node of output layer')
...
#Results dir
path_base = f"./exp/DIM10_Seed_{args.seed}_layers_{args.num_layers}_dim_hidden_{args.dim_hidden}_dim_out_{args.dim_out}/N_f_{args.N_f}_sign_{args.sign}"
...
for iter in range(2000):
    ...
    if iter >0 and iter %1 ==0: # 10
        with torch.no_grad():
            weit_f = torch.norm(X_f-1/2,dim=0,keepdim=True)
    ...
    if iter >0 and (iter+1) % 200 == 0:
        ...
        # EVALUATION code
        N_part = 100
        ind_sub = np.random.choice(N_dim,3,replace=False)        

        input_samp = [torch.linspace(left_end,right_end,N_part).to(device) for x in range(2)]
        input_ind = torch.meshgrid(*input_samp)
        # t = torch.linspace(left_end,right_end,N_part).flatten()[:,None]
        fun_eval = lambda x,y,z: torch.sin(nu*math.pi*x)*torch.sin(nu*math.pi*y)*torch.sin(nu*math.pi*z)
        ...
        for i, idx in  enumerate(ind_sub[:2]):
            ori_list[idx] = input_ind[i]
        z = np.linspace(0.0,0.95,20)
        Err_t = None
        for im, zi in enumerate(z):
            ori_list[ind_sub[-1]] = zi*torch.ones_like(input_ind[0])
            X_star = torch.stack([ele.flatten() for ele in ori_list],-1)
            data = fun_eval(input_ind[0],input_ind[1],zi*torch.ones_like(input_ind[0]))
            data = -args.sign*data
            u_star = data.flatten()[:,None]   
            ...
            if Err_t is None:
                ...
                Err_t += error_u
            sys.stdout = Logger(path_base+'/results.txt')
            print('Error u: %e FOR DIM = %d x %d' % (error_u,ind_sub[0],ind_sub[1]))                     
            ...
            ax.set_xlabel('$x$_%d'%(ind_sub[0],), size=20)
            ax.set_ylabel('$x$_%d'%(ind_sub[1],), size=20)
            ax.legend(
                loc='upper center', 
            ...
            plt.savefig('{}.png'.format(os.path.join(path_base,'out_iter%d_%d' %(iter,im))), bbox_inches='tight', pad_inches=0)
            plt.clf()
        sys.stdout = Logger(path_base+'/results.txt')
        print('Avg Error u: %e FOR DIM = %d x %d' % (Err_t/20,ind_sub[0],ind_sub[1]))
    if iter >0 and (iter+1) % 200 ==0:
        # save the model
